...
import { Youtube, Download, Loader as Loader2, CircleAlert as AlertCircle, CircleCheck as CheckCircle2, Link2, Clipboard, Video, ArrowDown } from 'lucide-react';
...
function BigDownloadButton({ item, label, isMain }: { item: DownloadItem; label: string; isMain?: boolean }) {
  ...
  downloads: [
    {
      ...
      localStorage.setItem('reelnest_history', JSON.stringify(
        [{ id: Date.now().toString(), url, platform: 'youtube', type: 'mp4', quality: q, timestamp: Date.now() },
          ...history].slice(0, 20)
      ));
    } catch (err: any) {
      fetchedRef.current = '';
...
const msg =
  ...
  useEffect(() => {
    ...
    if (p) { setInputUrl(p); fetchVideo(p, quality); }
    ...
    <div className="min-h-screen pt-24 pb-32 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <motion.div className="text-center mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-gray-400 text-sm">Paste link → one click → MP4 saved to device</p>
        </motion.div>
        ...
        <motion.div className="flex justify-center gap-2 mb-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          {QUALITY_OPTIONS.map(q => (
            <button
              key={q}
              onClick={() => { setQuality(q); fetchedRef.current = ''; setResult(null); }}
            ...
            <motion.div className="glass rounded-2xl overflow-hidden mb-8"
              ...
              {result.thumbnail && (
                <div className="relative aspect-video bg-black/60">
                  ...
                  <img src={result.thumbnail} alt={result.title || 'Preview'}
                    ...
                    <div className="mt-2 space-y-2">
                      {mp4Items.slice(1).map((item, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/5 border border-white/8">
                          ...
                          <button
                            onClick={async () => { await triggerDownload(item.url, `reelnest-youtube-${Date.now()}.mp4`); toast.success('Download started!'); }}
          ...
          <motion.div className="glass rounded-2xl p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
            ...
            <ol className="space-y-2.5">
              {[
                ...
                'Quality select karo (720p recommended)',
                '"Download" button click karo — file save hogi',
              ].map((step, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-gray-400">
                  <span className="w-5 h-5 rounded-full bg-[#FF0000]/20 text-[#FF0000] text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                  {step}
                </li>
              ))}
            </ol>
