...
import { Music, Download, Loader as Loader2, CircleAlert as AlertCircle, CircleCheck as CheckCircle2, Link2, Clipboard, Headphones, ArrowDown } from 'lucide-react';
...
async function triggerDownload(mediaUrl: string, filename: string) {
  ...
  a.href = `/api/download?url=${encodeURIComponent(mediaUrl)}&filename=${encodeURIComponent(filename)}`;
  a.download = filename;
  a.rel = 'noopener';
...
setResult({
  ...
  downloads: [
    {
      ...
      localStorage.setItem('reelnest_history', JSON.stringify(
        [{ id: Date.now().toString(), url, platform: 'mp3', type: 'mp3', quality: `${br}kbps`, timestamp: Date.now() },
          ...history].slice(0, 20)
      ));
    } catch (err: any) {
      fetchedRef.current = '';
      const msg = err.response?.data?.error || 'Failed to convert. Check the URL and try again.';
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    if (p) { setInputUrl(p); fetchAudio(p, bitrate); }
  ...
  const best = mp3Items.find(d => d.quality.startsWith(bitrate)) ?? mp3Items[0];
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <motion.div className="text-center mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-2">
            YouTube to <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-green-500">MP3</span>
          </h1>
          <p className="text-gray-400 text-sm">Paste YouTube link → one click → MP3 downloaded</p>
        </motion.div>

        {/* Bitrate selector */}
        <motion.div className="flex justify-center gap-2 mb-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          {BITRATES.map(br => (
            <button
              key={br}
              onClick={() => { setBitrate(br); fetchedRef.current = ''; setResult(null); }}
        ...
        {/* URL Input Card */}
        <motion.div className="glass rounded-2xl p-5 sm:p-6 mb-5" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            ...
            <div className="relative flex-1 group">
              ...
              <div className="relative flex items-center bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                ...
                <input
                  type="url"
                  value={inputUrl}
                  onChange={e => { setInputUrl(e.target.value); fetchedRef.current = ''; setResult(null); setError(''); }}
                ...
                />
                {inputUrl && (
                  <button onClick={() => { setInputUrl(''); setResult(null); setError(''); fetchedRef.current = ''; }}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
          >
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" />Converting…</>
                     : <><Music className="w-4 h-4" />Convert to MP3</>}
          </motion.button>
        </motion.div>
        ...
        <AnimatePresence>
          {error && (
            <motion.div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/20 mb-5"
        ...
        <AnimatePresence>
          {loading && (
            <motion.div className="glass rounded-2xl overflow-hidden mb-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        ...
        <AnimatePresence>
          {result && !loading && (
            <motion.div className="glass rounded-2xl overflow-hidden mb-8"
              ...
              {result.thumbnail && (
                <div className="relative aspect-video bg-black/60">
                  ...
                  <img src={result.thumbnail} alt={result.title || 'Audio'}
                ...
                {mp3Items.length > 1 && (
                  <details>
                    <summary className="text-xs text-gray-500 hover:text-gray-300 cursor-pointer select-none text-center py-1 transition-colors">
                      Other bitrate options ▾
                    </summary>
                    <div className="mt-2 space-y-2">
                      {mp3Items.filter(d => d !== best).map((item, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/5 border border-white/8">
                          <div>
                            <span className="text-sm font-medium text-white">{item.quality}</span>
                            <span className="text-xs text-gray-500 ml-2">{item.size}</span>
                          </div>
                          <button
                            onClick={async () => { await triggerDownload(item.url, `reelnest-audio-${Date.now()}.mp3`); toast.success('Download started!'); }}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 text-xs font-semibold rounded-lg transition-all"
                          >
        ...
        {!result && !loading && !error && (
          <motion.div className="glass rounded-2xl p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">How to convert</p>
            <ol className="space-y-2.5">
              {[
                'YouTube video open karo aur URL copy karo',
                'Upar input mein paste karo',
                'Apna bitrate choose karo (192 kbps recommended)',
