...
const INSTAGRAM_URL_REGEX =
  /https?:\/\/(www\.)?instagram\.com\/(p|reel|tv|reels|stories)\/[A-Za-z0-9_-]+/;
...
function BigDownloadButton({
  ...
  const handleClick = async () => {
    ...
    await triggerDownload(
      item.url,
      `reelnest-instagram-${Date.now()}.${item.format}`
    );
...
export default function InstagramPage() {
  ...
  const handleFetch = () => {
    ...
    if (!v) {
      toast.error('Please paste Instagram URL');
      return;
    }
